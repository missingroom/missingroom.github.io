...
对于数论函数 $f,g$，定义它们的狄利克雷卷积为 $(f*g)(x)=\sum\limits_{d\mid x}f(d)g(\frac{x}{d})$，换种容易理解的表示方式即 $(f*g)(x)=\sum\limits_{a\times b=x}f(a)g(b)$。
...
$n=1$ 时特殊处理一下，这里使用质因子次数和的相反数作为 $\ln x$，它与 $\ln x$ 有着类似的性质，我们并不在意 $\ln x$ 的真实值，所以直接用质因子次数和的相反数代替即可。
...
### [DIVCNT2](https://www.luogu.com.cn/problem/SP20173)

设函数 $f(i)=d(i^2)$，这个函数显然是积性函数。
$$
\begin{aligned}\\
&\prod\limits_{p\in\mathrm{Prime}}\sum_{i=0}^\infin\frac{2i+1}{p^{ix}}\\
=&\prod\limits_{p\in\mathrm{Prime}}\left(\sum_{i=0}^\infin\frac{2i}{p^{ix}}+\sum_{i=0}^\infin\frac{1}{p^{ix}}\right)\\
=&\prod\limits_{p\in\mathrm{Prime}}\left(\frac{2p^{-x}}{(1-p^{-x})^2}+\frac{1}{1-p^{-x}}\right)\\
=&\prod\limits_{p\in\mathrm{Prime}}\left(\frac{2p^{-x}+1-p^{-x}}{(1-p^{-x})^2}\right)\\
=&\prod\limits_{p\in\mathrm{Prime}}\frac{1+p^{-x}}{(1-p^{-x})^2}\\
=&\frac{\zeta(x)^3}{\zeta(2x)}
\end{aligned}\\
$$
显然，我们可以看出 $f=\mu^2*d$，$\sum\limits_{i=1}^n f_i=\sum\limits_{i=1}^n\mu^2_i\sum\limits_{j=1}^{\lfloor\frac{n}{i}\rfloor}d_j$。$d$ 的前缀和可以 $\mathrm O(\sqrt n)$ 算，预处理到 $\mathrm O(n^{\frac{2}{3}})$ 最优，$\mu^2$ 的前缀和可以看[这道题](https://www.luogu.com.cn/problem/SP4168)，复杂度分析类似杜教筛。

### [DIVCNT3](https://www.luogu.com.cn/problem/SP20174)

设函数 $f(i)=d(i^3)$，这个函数显然是积性函数。
$$
\begin{aligned}\\
&\prod\limits_{p\in\mathrm{Prime}}\sum_{i=0}^\infin\frac{3i+1}{p^{ix}}\\
=&\prod\limits_{p\in\mathrm{Prime}}\frac{1+2p^{-x}}{(1-p^{-x})^2}\\
=&\zeta(x)^2\prod\limits_{p\in\mathrm{Prime}}(1+2p^{-x})
\end{aligned}\\
$$
